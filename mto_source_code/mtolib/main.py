...
from mtolib.io_mto import generate_image, generate_parameters, read_fits_file, make_parser, write_fits_file
from mtolib.utils import time_function
from ctypes import c_float, c_double
from mtolib.postprocessing import relabel_segments
from mtolib.significance_tests import sig_test_1, default_sig_test
...
def max_tree_timed(img, params, maxtree_class, real_img=None):
    """Build and return a maxtree of a given class"""
    if params.verbosity:
        print("\n---Building Maxtree---")
    mt = maxtree_class(img, params.verbosity, params, real_img)
    mt.flood()
    return mt


def build_max_tree(img, params, maxtree_class=maxtree.OriginalMaxTree, real_img=None):
    return time_function(max_tree_timed, (img, params, maxtree_class, real_img), params.verbosity, 'create max tree')
