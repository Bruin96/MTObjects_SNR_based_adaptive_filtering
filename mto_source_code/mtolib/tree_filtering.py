...
def filter_tree(mt_in, image, params, sig_test=default_sig_test,
                sig_nodes_function=up_tree, get_sersics=False):
    if params.verbosity:
        print("\n---Finding Objects---")
    return time_function(filter_tree_timed, (mt_in, image, params, sig_test, sig_nodes_function, get_sersics),
                         params.verbosity, 'find objects')


def filter_tree_timed(mt_in, shape, params, sig_test=default_sig_test,
                sig_nodes_function=up_tree, get_sersics=False):
    ...
    object_ids = np.zeros(shape, dtype=ct.c_int32) - 1
    id_pointer = object_ids.ctypes.data_as(object_id_type)

    # Ditto for significant ancestors
    sig_ancs = np.zeros(shape, dtype=ct.c_int32) -3
    ...
    elif not hasattr(sig_test, 'test'):
        sig_test = sig_test(mto_lib)
