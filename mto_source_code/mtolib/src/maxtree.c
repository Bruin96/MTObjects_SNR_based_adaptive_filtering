...
int mt_x_coord(mt_data* mt, int pixel){
    return pixel % mt->img.width;
}

int mt_y_coord(mt_data* mt, int pixel){
    return pixel / mt->img.width;
}
...
static void mt_init_nodes(mt_data* mt){
  ...
  for (i = 0; i != mt->img.size; ++i){
    ...
    mt->nodes[i].area = 1;
    mt->nodes_attributes[i].min_branch_val = mt->img.data[i];
  }
}

static int mt_queue_neighbour(mt_data* mt, PIXEL_TYPE val, SHORT_TYPE x, SHORT_TYPE y){
...
static void mt_queue_neighbours(mt_data* mt, mt_pixel* pixel){
  ...
  for (conn_y = conn_y_min; conn_y <= conn_y_max; ++conn_y){
    ...
    for (conn_x = conn_x_min; conn_x <= conn_x_max; ++conn_x){
      ...
      if (mt->connectivity.neighbors[conn_y * mt->connectivity.width + conn_x] == 0){
        continue;
      }
...
static void mt_merge_nodes(mt_data* mt,  INT_TYPE merge_to_idx, INT_TYPE merge_from_idx){
    ...
    FLOAT_TYPE from_val = mt->real_img.data[merge_from_idx];
    FLOAT_TYPE delta = mt->real_img.data[merge_from_idx] -
      mt->real_img.data[merge_to_idx];
    ...
    merge_from_attr->sum += from_val;
    merge_to_attr->sum += merge_from_attr->sum;

    merge_from_attr->sumsq += (from_val*from_val);
    merge_to_attr->sumsq += merge_from_attr->sumsq;
    
    if(merge_from_attr->min_branch_val < merge_to_attr->min_branch_val){
        merge_to_attr->min_branch_val = merge_from_attr->min_branch_val;
    }
...
static void mt_descend(mt_data* mt, mt_pixel *next_pixel){
  ...
  if (mt->real_img.data[stack_top_index] >= mt->real_img.data[stack_top_index]){
    mt->nodes[old_top_index].parent = stack_top_index;
    mt_merge_nodes(mt, stack_top_index, old_top_index);
  }else{
    mt->nodes[stack_top_index].parent = old_top_index;
    mt_merge_nodes(mt, old_top_index, stack_top_index);
...
static void mt_remaining_stack(mt_data* mt){
  while (MT_STACK_SIZE(&mt->stack) > 1)
  {
    ...
    if (mt->real_img.data[stack_top_index] >= mt->real_img.data[old_top_index]){
      mt->nodes[old_top_index].parent = stack_top_index;
      mt_merge_nodes(mt, stack_top_index, old_top_index);
    }else{
      mt->nodes[stack_top_index].parent = old_top_index;
...
void mt_init(mt_data* mt, const image* img, const image* real_img){
  mt->img = *img;
  mt->real_img = *real_img;

  mt->nodes = safe_malloc(mt->img.size * sizeof(mt_node));
  mt->nodes_attributes = safe_calloc(mt->img.size, sizeof(mt_node_attributes));
...
void mt_print_node_stats(mt_data* mt, float gain, float bg_variance){
  // Iterate over image pixels
  SHORT_TYPE y;
  for (y = 0; y != mt->img.height; ++y)
  {
    ...
    for (x = 0; x != mt->img.width; ++x)
    {
      ...
      FLOAT_TYPE min_val = mt->nodes_attributes[parent_idx].min_branch_val;//attr->min_branch_val;
      FLOAT_TYPE min_val2 = min_val * min_val;
      FLOAT_TYPE new_variance = (min_val / gain) + bg_variance;
      ...
      printf("%.5e,%.5e,%d,%.5e,%.5e,%.5e,%.5e\n",
            attr->sumsq,
            attr->sum,
            area,
            mt->real_img.data[i],
            min_val,
            new_power_normalized,
            gsl_cdf_chisq_Q(new_power_normalized, area)
        );
    }
  }
}
