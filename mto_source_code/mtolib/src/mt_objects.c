...
void mt_update_parent_main_branch(mt_object_data* mt_o, INT_TYPE node_idx){
  ...
  INT_TYPE ancestor_idx = mt_o->closest_significant_ancestors[node_idx];
  ...
  {
    if (mt->nodes[mt_o->main_branches[ancestor_idx]].area < mt->nodes[node_idx].area)
    {
...
void mt_find_objects(mt_object_data* mt_o){
  ...
  for (i = 0; i != mt->img.size; ++i){
    ...
    if (!MT_HAVE_SIGNIFICANT_ANCESTOR(i)){
      ...
      // Add pixel to heap
      mt_pixel pixel;
      pixel.location.x = mt_x_coord(mt, i);
      pixel.location.y = mt_y_coord(mt, i);
      pixel.value = mt->nodes[i].area;

      continue;
    }

    // Get the parent node, to check if this node is on the main branch
    INT_TYPE parent = mt_o->closest_significant_ancestors[i];

    // If this node is not its parent's main branch, mark it as an object (nested)
    if (mt_o->main_branches[parent] != i){
      ...
      pixel.location.x = mt_x_coord(mt, i);
      pixel.location.y = mt_y_coord(mt, i);
      pixel.value = (FLOAT_TYPE)mt->nodes[i].area;
    }
    continue;
  ...
  if (mt->verbosity_level){
    printf("Found %d objects (%d nested).\n", num_objects, num_objects_nested);
  }
...
void mt_move_up(mt_object_data* mt_o){
  ...
  {  
    ...
    FLOAT_TYPE base = mt->real_img.data[i] - MT_DISTANCE(i);
    ...
    while (mt->real_img.data[next_idx] < base)
    { 
...
void mt_objects_init(mt_object_data *mt_o){
  ...
  mt_o->main_branches = safe_malloc(img_size * sizeof(*mt_o->main_branches));

  mt_o->main_power_branches = safe_malloc(img_size * sizeof(*mt_o->main_power_branches));
...
FLOAT_TYPE mt_noise_variance(mt_object_data* mt_o,
  ...
  {
    variance +=
      mt->real_img.data[mt_o->closest_significant_ancestors[node_idx]] /
      mt_o->paras->gain;
  }
  ...
  if (max_normalized_distance >= 0)
  {
    ...
    if (distance / rms > max_normalized_distance)
    {
      ...
      FLOAT_TYPE b = 2 * mt->real_img.data[node_idx] * mt_o->paras->gain;
...
FLOAT_TYPE mt_alternative_power_definition(mt_object_data* mt_o,
  ...
  FLOAT_TYPE delta;
  if (MT_HAVE_SIGNIFICANT_ANCESTOR(node_idx)){
    delta = mt->real_img.data[parent_idx] - mt->real_img.data[mt_o->closest_significant_ancestors[node_idx]];
  }
  else{
    delta = mt->real_img.data[parent_idx];
  }
  ...
  if (max_normalized_distance >= 0){
    FLOAT_TYPE rms = sqrt(mt_noise_variance(mt_o, node_idx, max_normalized_distance));
    ...
    if (distance / rms > max_normalized_distance){
      delta = max_normalized_distance * rms - mt->real_img.data[parent_idx];
